...
const { configManager } = maestro.managers;

export default class GameProcessBuilder {
  constructor(distroServer, versionData, forgeData, authUser, launcherVersion) {
    this.gameDir = path.join(appSystemHelper.getInstanceDirectory(), distroServer.getID());
    this.commonDir = appSystemHelper.getCommonDirectory();
  ...
  build(javaExecutablePath) {
    fs.ensureDirSync(this.gameDir);

    const tempNativePath = path.join(
      os.tmpdir(),
      appSystemHelper.getTempNativeFolder(),
    ...
    const modObj = this.resolveModConfiguration(
      configManager.getModConfiguration(this.server.getID()).mods,
      this.server.getModules(),
    );
    ...
    console.log('Java Executable', javaExecutablePath);
    console.log('Launch Arguments:', args);

    const child = childProcess.spawn(javaExecutablePath, args, {
      cwd: this.gameDir,
  ...
  setupLiteLoader() {
    for (const ll of this.server.getModules()) {
      if (ll.getType() === MODULE_TYPES.LiteLoader) {
        if (!ll.getRequired().isRequired()) {
          // const modCfg = configManager.getModConfiguration(this.server.getID()).mods;

          // if (GameProcessBuilder.isModEnabled(modCfg[ll.getVersionlessID()], ll.getRequired())) {
          // if (true) {
  ...
  resolveModConfiguration(modCfg, mdls) {
    ...
    for (const mdl of mdls) {
      ...
      if (type === MODULE_TYPES.ForgeMod || type === MODULE_TYPES.LiteMod || type === MODULE_TYPES.LiteLoader) {
        const o = !mdl.getRequired().isRequired();
        const e = false; //GameProcessBuilder.isModEnabled(modCfg[mdl.getVersionlessID()], mdl.getRequired()); // TODO ENABLED MODS
        ...
        if (!o || (o && e)) {
          if (mdl.hasSubModules()) {
            ...
            if (mdl.type === MODULE_TYPES.LiteLoader) {
              continue;
            }
          }

          if (mdl.type === MODULE_TYPES.ForgeMod) {
            fMods.push(mdl);
          } else {
            lMods.push(mdl);
          }
  ...
  _constructJVMArguments113(mods, tempNativePath) {
    ...
    for (let i = 0; i < args.length; i++) {
      if (typeof args[i] === 'object' && args[i].rules != null) {
        ...
        for (const rule of args[i].rules) {
          if (rule.os != null) {
            if (rule.os.name === Library.mojangFriendlyOS() && (rule.os.version == null || new RegExp(rule.os.version).test(os.release))) {
    ...
    try {
      ...
      console.error(err);
      console.error('Forge version format changed.. assuming autoconnect works.');
      console.debug('Forge version:', this.forgeData.id);
    }

    if (isAutoconnectBroken) {
      console.error('Server autoconnect disabled on Forge 1.15.2 for builds earlier than 31.2.15 due to OpenGL Stack Overflow issue.');
      console.error('Please upgrade your Forge version to at least 31.2.15!');
    } else {
  ...
  _resolveServerLibraries(mods) {
    ...
    for (const mdl of mdls) {
      ...
      if (type === MODULE_TYPES.ForgeHosted || type === MODULE_TYPES.Library) {
  ...
  _resolveModuleLibraries(mdl) {
    ...
    for (const sm of mdl.getSubModules()) {
      if (sm.getType() === MODULE_TYPES.Library) {
